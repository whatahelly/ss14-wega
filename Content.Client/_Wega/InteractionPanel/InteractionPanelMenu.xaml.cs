...
namespace Content.Client.Interaction.Panel.Ui
{
    ...
    public sealed partial class InteractionPanelMenu : DefaultWindow
    {
        ...
        private NetEntity? FindTarget(EntityUid user)
        {
            ...
            foreach (var humanoidEntity in nearbyEntities)
            {
                var humanoid = humanoidEntity.Owner;
                if (humanoid == user) continue;

                if (!_entManager.TryGetComponent<TransformComponent>(humanoid, out var userTransform))
                    continue;

                var sessionForHumanoid = playerManager.Sessions.FirstOrDefault(session => session.AttachedEntity == humanoid);
                if (sessionForHumanoid == null)
                    continue;

                var humanoidWorldPosition = _transform.GetWorldPosition(humanoid);
                var distance = Vector2.Distance(sourceWorldPosition, humanoidWorldPosition);
                if (distance <= minDistance)
                {
                    minDistance = distance;
                    target = _entManager.GetNetEntity(humanoid);
                }
            }
            ...
            if (target is null)
            {
                ...
                foreach (var strapEntity in strapEntities)
                {
                    ...
                    {
                        if (buckledEntity == user) continue;
